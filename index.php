	...
	<head>
		...
		<script>
			...
			app.controller("jn-prestations_Ctrl", function($scope, $http, $window,  $interval){		
				...
				$scope.firstNameFormatInvalid = "Le nom ne doit contenir que des lettres; sans espace !";
				...
				$http.get("api/listAllContacts.php")
				...
				$scope.user = {};
				 ...
				 function isEmptyOrNull(elem){
					return (elem == undefined || elem == null);
				}
				$scope.submit_form = function(){
					$scope.success = false;
					$scope.failure = false;
					if(isEmptyOrNull($scope.user.firstName)){
						$scope.user.firstName = ' ';
					}
					if(isEmptyOrNull($scope.user.status)){
						$scope.user.status = ' ';
					}
					if(isEmptyOrNull($scope.user.purpose)){
						$scope.user.purpose = ' ';
					}
					if(isEmptyOrNull($scope.user.message)){
						$scope.user.message = ' ';
					}	
					$http({
						method:'POST', 
						url:'api/newContact.php',
				...
				$scope.contactsTableCheckbox = false;     
				$scope.process_adminForm = function(){
					$http.get("api/credentials.php")
		...
		*/
		...
		<style type="text/css">					
			* {margin: 0;border: 0;padding: 0;}
			.container {background: #fafafa;padding: 0;}
			body {font-family: Arial, Helvetica, sans-serif;color: #555;background: #A9A9A9;margin: 15px;}
			#slideShow {width: 100%;height: 150px;padding: 10px;}
			#slideShow>div {position: absolute;}
			h1 {text-align: center;font-size: 180%;padding: 2% 0;}
			h4 {text-align: center;}
			h5 {background-color: gray;color: white;text-align: center;height: 22px;}
			p {padding: 1%;}
			...
			.navbar-brand{width: 350px;height: 90px;float: left;background: url(images/logo) no-repeat center;		}
			nav{float: right;padding: 20px 20px 20px 0;font-size: 16px;}
			ul{list-style: none;}
			nav ul li{display: inline-block;float: left;padding: 10px;height:60px;}
			.servicesArticle{width: 45%;float: left;margin: 2% 2%;text-align: left;}
			#servicesArticleId{margin-left: 4%;}
			.qualityArticle{width: 28%;float: left;margin: 2% 2%;text-align: left;}
			#lobby{float: right;}
			#hedge{float: left;margin-right: 20px;}
			#cleaning, #greenSpaces{width: 70%;height: 50%;}
			#proximity, #quality, #ecology{margin-left: 25%;width: 30%;height: 25%;}
			#contact{width: 15%;height: 10%;float: right;}
			.corporate{color: #0000e8;}
			.corporateInfos, .warning, .corporate{font-weight: bold;}
			.redStar, .error{color:red;}
			legend{color: blue;}
			.warning{margin-left: 35%;}
			form{width: 100%;}
			fieldset{background-color: #e5e5e5;border: 2xp solid #e5e5e5;padding: 3px;}
			.textInput{margin-top: 4px;width: 45%;height: 40px;border-radius: 4px;border: 2px solid #ccc;}
			.buttonInput{background-color: #d7ffff;height: 25px;}
			.textLabel{float: left;width: 150px;}
			#demand{width: 60%;height: 40px;border-radius: 4px;border: 2px solid #ccc;}
			#message{resize:vertical;width: 60%;height: 200px;border-radius: 4px;border: 2px solid #ccc;}
			#send{margin-left: 30%;margin-right: 20px;}
			#familyName_error, #phoneNumber_error, #email_error{color: red;margin-left: 0%;}
			.alert{width: 60%;}
			footer{background: #405580;width: 100%;height: 25%;}
							...
							<p>
								...
								<form name="contactForm" novalidate> 
									<fieldset>
										...
										Vous êtes:<br />
										<input ng-model="user.status" type="radio" name="status" class="corporate" value="une entreprise" />
			...
			<section>
				<div>
					...
					<form ng-show="contactsTableCheckbox">
						<span class="alert alert-info"> Show the contacts table
					...
					<table ng-show="showTable" class="table table-bordered">
						...
						<tbody>
							<tr ng-repeat="user in users" >
								...
								<td>{{user.purpose}}</td>
								<td>{{user.message}}</td>
								<td><button type="button" class="btn btn-danger" ng-click="deleteContact(user.position)">delete</button></td>
							</tr>
						</tbody>
					</table>
				</p>
